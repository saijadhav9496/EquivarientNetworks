...
def train(train_loader, device, model, criterion, optimizer, summary_writer, train_state, flag, chairs, n_classes=None):
    ...
    with open(PIK, "wb") as f:
        ...
        for i, (inputs, targets, fnames, tload) in enumerate(train_loader):
            ...
            outputs,descriptor = model(inputs)

            #print("k---",k)   
            #print(descriptor.shape,inputs.shape)

            if(flag==1):
                pickle.dump((inputs.cpu().detach().numpy(),descriptor.cpu().detach().numpy()), f)
                #descriptors.append((inputs.cpu().detach().numpy(),descriptor.cpu().detach().numpy()))
                '''
                idx = 0
                for f in fnames:
                    word = str(f).split("/")[-2]
                    if word in chairs:
                        print("found!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        #idx = chairs.index(word)
                        features[word] = np.array(descriptor.cpu().detach().numpy()[idx,:])
                    idx = idx + 1
                    '''
...
def eval(data_loader, device, model, criterion, eval_shrec,flag,chairs,features):
    #model = torch.load("saved.pth")
    #model.eval()
    def _acc(preds, tgts):
                ...
                if(flag==1):
                    ...
                    for f in fnames:
                        ...
                        if word in chairs:
                            print("found!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                            #idx = chairs.index(word)
                            feat = np.array(descriptor.cpu().detach().numpy()[idx,:])
                            min_diff = sys.maxsize

                            for key,val in features.items():
                                print(key,"-------",LA.norm(feat - val))
                                if LA.norm(feat - val) < min_diff:
                                    min_diff = LA.norm(feat - val)
                                    best_match = key
                            print("The best matching model is : ", best_match)
                        idx = idx + 1
...
def main():
    train_chairs = ['chair_0001','chair_0005','chair_0101','chair_0084','chair_0497','chair_0724','chair_0878']
    ...
    for epoch in range(0, n_epochs):
        ...
        if not args.skip_train:
            ...
            if epoch == n_epochs - 1:
                features = train(train_loader, device, model, criterion, optimizer, summary_writer, train_state,1,                               train_chairs,n_classes=len(classes))
                ...
                train(train_loader, device, model, criterion, optimizer, summary_writer, train_state,0, train_chairs,
        ...
        if not args.skip_eval or epoch == n_epochs + 1:
            ...
            metrics, inputs = eval(
                val_loader, device, model, criterion, eval_shrec,0,test_chairs,features)
    ...
    for epoch in range(0, 12):
        print("Epoch ", epoch+1)
        model.train()
        for i, (target_img, labels, fnames, tload) in enumerate(train_loader):
            #img = target_img.cpu().detach().numpy()
            if(i==0):
                print(fnames)
            img = target_img[1,1,:,:]
            img = Image.fromarray(img)
            img.save('m1.png')
            target_img, labels = load_inputs_targets(target_img, labels, device)
            
            inputs = descriptors[i]
