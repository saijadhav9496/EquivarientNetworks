...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.pool1 = nn.Upsample(20)
        self.pool2 = nn.Upsample(7)
        ...
        self.pool3 = nn.Upsample(112)

        self.deconv5 = nn.ConvTranspose2d(64,3,4,stride=2,padding = 1)
    ...
    def forward(self,x):
        ...
        x = self.pool1(x)
        shp = x.shape
        x = x.view((shp[0] * shp[2] , shp[1], 1, 1))
        x = self.pool2(x)
        ...
        shp = x.shape
        x = x.view((12,20,shp[1] ,shp[2], shp[3]))
        return x
...
class ResNetMVGCNN(nn.Module):
    ...
    def __init__(self, block, channels, gconv_channels,
        ...
        print("gconv channels",gconv_channels)
    ...
    def forward(self, x):
        ...
        if 'verylate' not in self.viewpool:
            ...
            y = x
            #print("final desc----------",x.shape)
            x = self.gcnn_fc(x)
            #print("gcnnfc----------",x.shape)
